...
- ## 当前任务 → конкретное действие, которое эта глава обязана завершить; сверяйся с ним на всех этапах.
- ## 读者此刻在等什么 → задаёт, как создавать / задерживать / оплачивать эмоциональные разрывы.
- ## 该兑现的 / 暂不掀的 → крючки, которые обязаны закрыться в этой главе + карты, которые НЕЛЬЗЯ раскрывать.
- ## 日常/过渡承担什么任务 → карта функций для неконфликтных пассажей ([место в главе] → [функция]).
- ## 关键抉择过三连问 → тройной вопрос, через который должен пройти каждый ключевой выбор персонажа.
- ## 章尾必须发生的改变 → 1–3 конкретных сдвига, которые должна обеспечить концовка (информация / отношения / физическое состояние / власть).
- ## 不要做 → жёсткие запреты на эту главу.
