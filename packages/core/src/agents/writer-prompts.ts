...
import { buildRussianCoreRules, buildRussianAntiAIRules, buildRussianCharacterMethod, buildRussianPreWriteChecklist, buildRussianGenreIntro } from "./ru-prompt-sections.js";
...
export function buildWriterSystemPrompt(
  ...
  const resolvedLanguage = (languageOverride ?? genreProfile.language) ?? "zh";
  ...
  const russianSections: string[] = [
    buildRussianGenreIntro(book, genreProfile),
    buildRussianCoreRules(book),
    buildGovernedInputContract("ru", governed),
    buildChapterMemoContract("ru", governed),
    buildLengthGuidance(resolvedLengthSpec, "ru"),
    buildWritingCraftCard("ru"),
    buildRussianAntiAIRules(),
    buildRussianCharacterMethod(),
    buildCreativeConstitution("ru"),
    buildImmersionPillars("ru"),
    buildGoldenOpeningDiscipline(chapterNumber, "ru"),
    buildRussianPreWriteChecklist(book, genreProfile),
  ...
  const sections = isRussian ? russianSections : isEnglish ? englishSections : chineseSections;
...
- In multi-character scenes, include at least one resistance-bearing exchange instead of reducing the beat to summary or explanation.`;
  ...
  if (language === "ru") {
    return `## Контракт управления входами

- Конкретное направление этой главы задаётся chapter intent и собранным context package.
- volume_outline — это план по умолчанию, а не безусловный высший закон.
- Когда runtime rule stack фиксирует активный L4 → L3 override, исполняй текущую задачу, а не локальное планирование.
- Жёсткие ограничители держи компактно: канон, факты непрерывности, явные запреты — всё ещё имеют приоритет.
- Если приходит Variance Brief, соблюдай его: избегай перечисленных штампованных фраз / зачинов / концовок и закрывай scene obligation.
- Если приходят Hook Debt Briefs, в них — ОРИГИНАЛЬНЫЙ ТЕКСТ-ЗЕРНО из той главы, где крючок был заложен. Используй этот текст для написания сцены-продолжения или закрытия так, чтобы для читателя возникла прямая связь с тем, что он уже видел — не размытое упоминание, а сцена, опирающаяся на конкретное обещание.
- Когда явная hook agenda называет правомочную цель закрытия, посади в эту главу конкретный payoff-такт, отвечающий на исходный вопрос читателя из главы-зерна.
- Если есть stale debt, не открывай однотипные параллельные крючки; сначала сними давление со старых обещаний, потом плоди новые.
- В сценах с несколькими персонажами обязательно одну стычку с реальным сопротивлением — не сводить такт к пересказу или объяснению.`;
  }
...
Address each section in order when drafting the chapter. Every section must leave a visible trace in the prose — if a section is not reflected, the chapter is incomplete.`;
  ...
  if (language === "ru") {
    return `## Согласование с памяткой главы

Тебе придёт chapter_memo из 7 markdown-разделов:

- ## Текущая задача → конкретное действие, которое эта глава обязана завершить; сверяйся с ним на всех этапах.
- ## Чего сейчас ждёт читатель → задаёт, как создавать / задерживать / оплачивать эмоциональные разрывы.
- ## Закрыть / Не открывать → крючки, которые обязаны закрыться в этой главе + карты, которые НЕЛЬЗЯ раскрывать.
- ## Что несут спокойные / переходные такты → карта функций для неконфликтных пассажей ([место в главе] → [функция]).
- ## Тройной вопрос по ключевому выбору → тройной вопрос, через который должен пройти каждый ключевой выбор персонажа.
- ## Обязательные сдвиги к финалу главы → 1–3 конкретных сдвига, которые должна обеспечить концовка (информация / отношения / физическое состояние / власть).
- ## Не делать → жёсткие запреты на эту главу.

Закрывай разделы по порядку. Каждый раздел должен оставить видимый след в прозе — если раздел не отражён, глава считается незавершённой.`;
  }
...
- Жёсткий диапазон: ${lengthSpec.hardMin}–${lengthSpec.hardMax} слов`;
  }
...
function buildWritingCraftCard(language: "zh" | "en" | "ru"): string {
  if (language === "ru") {
    return `## Писательские правила (всегда в силе)

- **Эмоция**: выноси наружу через действие — не «он почувствовал гнев», а «он раздавил чашку, кипяток потёк сквозь пальцы».
- **Соль в супе**: ценности — через поведение, а не через лозунги.
- **Второстепенные**: у каждого свой расчёт. Протагонист побеждает, переигрывая умных, а не уничтожая дураков.
- **Пять чувств**: мокрая футболка прилипает к спине, запах больничной хлорки, лужи на остановке.
- **Конкретика**: не «большой город» — «заднее сиденье такси, простоявшего сорок минут на МКАДе».
- **Синтаксис**: меньше «однако / тем не менее / вместе с тем / следует подчеркнуть»; вместо связок — внутренний голос или жест персонажа.
- **Двигатель желания**: создаёшь эмоциональный разрыв → читатель ждёт разрядку → разрядка ОБЯЗАНА превзойти ожидание. 70% удовлетворения = провал.
- **Тройной вопрос характера**: перед каждым действием — зачем? согласуется ли с предыдущими сценами? выглядит ли резко для читателя предыдущих глав?
- **Диалог**: разные персонажи говорят по-разному — лексика, длина фраз, словечки, региональные следы.
- **Запрещено**: представление персонажа в формате «карточки» / введение более 3 новых персонажей за главу / коллективные реакции типа «все ахнули».
- **Эскалация**: плохое складывается слоями, каждый слой хуже предыдущего. Не одна неприятность, а цепочка: упрёк → телефон упал в лужу → опоздал на собеседование → перебили посреди оправдания.
- **Цикл**: если сейчас фаза накопления давления — закладывай новые препятствия и информацию; если фаза взрыва — пиши payoff выше ожидания; если фаза последствий — пиши, кто что потерял, кто что получил, как сдвинулись отношения.
- **После пика**: после кульминации не прыгай сразу в новое накопление. Следующие 1–2 главы — про сдвиг: цена заплачена, статус изменился, новая нормальность установлена.
- **Управление ожиданием**: когда читатель жаждет разрядки — задерживай (это усиливает payoff); когда он на грани потерять интерес — давай отдачу немедленно.
- **Граница информации**: что этот персонаж сейчас знает? чего не знает? в чём ошибается? Персонаж действует только на основе того, что ему доступно.`;
  }
...
function buildCreativeConstitution(language: "zh" | "en" | "ru"): string {
  if (language === "ru") {
    return `## Творческая конституция

Эти четырнадцать принципов — твой хребет. Внутри тебя они работают всегда. Не цитируй их, не перечисляй, не пересказывай в тексте. Их назначение — помогать выбирать между двумя одинаково правдоподобными «следующими предложениями».

Show, don't tell — складывай реальную деталь так, чтобы правда видна, а не оглашена. Ценности растворяй в действии, как соль в супе: убеждение доказывается тем, что персонаж делает, когда никто не смотрит. Любое действие любого персонажа стоит одновременно на трёх ногах: прошлый опыт, текущий интерес, ядро характера; убери одну ногу — и сцена читается как авторский произвол. У каждого второстепенного — собственный реестр и собственный денежный мотив; они существовали до встречи с протагонистом и продолжат жить после. Темп — это дыхание: медленный огонь варит насыщенный бульон, бытовые сцены работают приманкой для основной линии, а не наполнителем. Каждая глава заканчивается небольшим крючком или эмоциональным разрывом — читатель должен хотеть следующую страницу. На сцене все умные одновременно: никакой удобной глупости, святошества и неподготовленных компромиссов. Отсылки к будущему звучи в голосе той эпохи, в которой ты их размещаешь. Хронология и быт эпохи не гнутся. Семьдесят процентов бытовых сцен должны параллельно сеять семена для основной линии. Изменения отношений нужны события — никаких «вмиг породнились», никаких «внезапно полюбил». Характер удерживается через всю арку; рост виден через работу. Ключевые сюжетные такты и завязки получают сцену, а не суммари. Откажись от драйфа в перечень: каждая строка либо двигает сюжет, либо доточивает человека.`;
  }
  if (language === "en") {
...
function buildImmersionPillars(language: "zh" | "en" | "ru"): string {
  if (language === "ru") {
    return `## Шесть опор погружения

Погружение читателя стоит на шести опорах. Установи их в первых нескольких страницах каждой сцены — молча, без названий, не объявляя.

Размечай базу: за первые сотню слов читатель должен знать, кто на сцене, где сцена и что происходит, чтобы он мог собрать комнату у себя в голове. Тянись к видимой узнаваемости: давай конкретику земного уровня, которую читатель сам трогал в жизни — запах больничной хлорки, холодное сиденье в метро, скрип пакета из доставки, — чтобы сцена догрузилась раньше, чем закончится второй абзац. Резонанс зарабатывай дважды — когнитивный («я бы сделал тот же выбор») и эмоциональный (родственное тепло, гнев на несправедливость, скорбь, тихая гордость). Питай желание двумя дорожками: базовые хотелки (получить даром, оказаться выше тех, кто давил, наконец выдохнуть) и активную хотелку, которую глава засевает сама — эмоциональный разрыв, который читатель уносит в следующую главу. Сажай сенсорные крючки: в каждой сцене 1–2 чувства помимо зрения (звук, запах, прикосновение, вкус), вплетены вскользь, никогда не «абзац погоды». Делай персонажей живыми через «основной штрих + одна контрастная деталь» — холодный ликвидатор подкармливает дворовых кошек, тёплый отец шутит так, что задевает, как нож. Эти опоры — форма любой сцены по умолчанию, а не пункты в чек-листе под конец главы.`;
  }
  if (language === "en") {
...
Это глава ${chapterNumber} из тройки открытия — твоя проза напрямую решает, останется ли читатель. Правило золотых трёх глав — не совет, а жёсткое ограничение для твоих предложений. Глава 1: в первых 800 словах протагонист обязан зацепиться за основной конфликт (преследование, тупик, лишение, перемещение-как-кризис); длинных абзацев предыстории нельзя, и устройство мира раскрывается через действия протагониста, а не объясняется блоком. Глава 2: преимущество — сила, система, память переродившегося, информационный перевес — должно быть **продемонстрировано** (один конкретный эпизод применения с видимым последствием), а не **объявлено** (абзац рассказчика о том, что оно есть). Глава 3: где-то в этой главе должна выйти на поверхность следующая измеримая краткосрочная цель протагониста, чтобы читатель закрыл страницу с конкретным «что дальше».

Сквозная дисциплина первых трёх глав: абзацы по 3–5 строк (мобильный ритм чтения), глаголы важнее прилагательных, и каждая глава кончается малым крючком — обрыв, нерешённый вопрос или эмоциональный разрыв. Максимум три сцены и максимум три названных персонажа в этой главе (лишние — только имя, без раскрытия). Информация послойно вшивается в действие: базовые факты (внешность, статус, положение) проступают из того, что делает протагонист; ключевые правила мира (механика системы, глубинная логика) привязываются к сюжетным триггерам; абзац чистой экспозиции запрещён.`;
  }
