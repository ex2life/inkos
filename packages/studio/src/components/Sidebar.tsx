...
{/*            <SidebarItem
      ...
      >
        ...
        >
          <DialogHeader className="space-y-0 gap-0">
            <DialogTitle className="font-sans text-sm font-medium">{t("sidebar.renameTitle")}</DialogTitle>
          </DialogHeader>
          <input
      ...
      <ConfirmDialog
        ...
        message={t("sidebar.deleteSessionMessage").replace("{title}", deleteTarget?.title ?? "")}
...
function getSessionLabel(
  ...
  // The backend persists the first user message as a placeholder title on send.
  // This branch handles the short-lived intermediate state — message exists locally
  // but the synced title has not arrived yet (optimistic display).
  const firstUserMsg = session.messages.find((m) => m.role === "user")?.content?.trim();
  if (firstUserMsg) {
    const oneLine = firstUserMsg.replace(/\s+/g, " ");
    return oneLine.length > 20 ? `${oneLine.slice(0, 20)}…` : oneLine;
  }
  return t("sidebar.newSessionFallback");
}
...
function formatRelativeTime(sessionId: string, t: TFunction): string {
  ...
  if (days < 30) return t("sidebar.daysAgo").replace("{n}", String(days));
  const months = Math.floor(days / 30);
  return t("sidebar.monthsAgo").replace("{n}", String(months));
}
