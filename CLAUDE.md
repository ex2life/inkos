...
- **防止 maxTokens 回归** — 替换 LLM provider / 改 `createLLMClient` 时必须遵守以下设计，违反了会让 agent per-call 指定的大 maxTokens 被意外裁剪、基础设定/章节输出被截断：
  - `LLMConfig.maxTokens` 的语义是 **per-call fallback**（agent 调 chat() 没指定 maxTokens 时用这个），**不是硬上限**
  - `LLMConfig.maxTokensCap` 的语义才是 **硬上限**——只有用户确实想封顶 per-call 最大输出时才显式 set，默认 undefined → `client.defaults.maxTokensCap = null` = 不封顶
  - 两个字段在 `createLLMClient` 里的赋值**禁止合并**：`maxTokens: config.maxTokens ?? 8192`、`maxTokensCap: config.maxTokensCap ?? null`；**严禁**写成 `maxTokensCap: config.maxTokens ?? null`（历史 bug，会让 architect 的 per-call 16384 被 config.maxTokens=8192 裁到 8192）
  - 任何改动必须让 `packages/core/src/__tests__/provider.test.ts` 里 "createLLMClient maxTokensCap regression" 那组回归测试继续通过
  - agent 内部调 `this.chat(..., { maxTokens: N })` 时 N 要反映该 agent 实际产出长度需求；**不要硬编码到 defaults**、**不要在 stub 测试里写具体数字误导后人**（stub 用 `as unknown as LLMClient` cast 即可）
